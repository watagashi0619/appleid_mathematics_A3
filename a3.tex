...
\[
	\hat f_1(\xi)=\frac{1}{\sqrt{2\pi}}\frac{2\alpha}{\alpha^2+\xi^2}
\]
\[
	\check f_1(t)=\frac{1}{\sqrt{2\pi}}\frac{2\alpha}{\alpha^2+t^2}
\]
...
\[
	\hat f_2(\xi)=\frac{\sqrt{2\pi}}{2\alpha}e^{-\alpha|\xi|}
\]

\[
	\check f_2(t)=\frac{\sqrt{2\pi}}{2\alpha}e^{-\alpha|t|}
\]
...
\[
	\hat f_3(\xi)=\frac{1}{\lambda}e^{-\xi^2/2\lambda^2}
\]
\[
	\check f_3(t)=\frac{1}{\lambda}e^{-t^2/2\lambda^2}
\]
...
\[
	\hat f_4(\xi)=\frac{2a}{\sqrt{2\pi}}\frac{\sin(a\xi)}{a\xi}
\]

例\ref{ex:3-1},\ref{ex:3-2},\ref{ex:3-3}は反転公式が成立するが，例\ref{ex:3-4}は反転公式が成立しない．
...
\footnotetext{教科書と異なり$f'$が有界であるという条件を付加しているが，これは証明中でこの条件が必要となるためである．\ref{sec:3-7}節を参照．}
...
\begin{prop}\label{prop:3-14}
	$f$が有界かつ局所可積分関数で，$g\in L^1(\mathbb{R})$ならば，$f*g$は有界な連続関数である．
\end{prop}
...
\begin{proof}
	...
	\[
		|(f*g)(t)|\leq\int_{-\infty}^\infty |f(t-s)g(s)|ds\leq \sup|f|\int_{-\infty}^\infty |g(s)|ds<+\infty
	\]
	より$f*g$は有界．$g\in L^1(\mathbb{R})$なので，
	\[
		\forall\varepsilon>0,\exists R>0, \int_{|s|>R}|g(s)|ds\leq\frac{\varepsilon}{4\sup|f|+1}
	\]
	とできる．（先はまだ...）
\end{proof}

\begin{prop}\label{prop:3-15}
	$f, g\in L^1(\mathbb{R})$が有界な台を持つ有界関数とする．このとき，$f*g$は有界な台を持つ連続関数である．
\end{prop}
...
\begin{prop}[畳み込みと微分との関係]\label{prop:3-16}
...
\begin{proof}
	微分と積分の順序交換ができる条件を確かめる．詳細は省略．
\end{proof}

\begin{thm}[たたみこみとフーリエ変換の関係]\label{thm:3-17}\ \vspace{-\baselineskip}
		...
		\item $f, g\in L^1(\mathbb{R})$が有界ならば
		      \[
			      \begin{split}
				      ...
				      \mathcal{F}^*[f*g](t)&=\sqrt{2\pi}\check{f}(t)\check{g}(t)
			      \end{split}
		      \]
		\item $f, g, \hat{g}\in L^1(\mathbb{R})$が有界ならば
		      \[
			      \begin{split}
				      \mathcal{F}[fg](\xi)&=\frac{1}{\sqrt{2\pi}}(\hat{f}*\hat{g})(\xi) \\
				      \mathcal{F}^*[fg](t)&=\frac{1}{\sqrt{2\pi}}(\check{f}*\check{g})(t)
			      \end{split}
		      \]
	\end{enumerate}
\end{thm}
...
\begin{empheq}[left = {\empheqlbrace \,}, right = {}]{align*}
	&\pdv{u}{t}(x,t)=\pdv[2]{u}{x}(x,t) && (x\in\mathbb{R},t>0) \\
	&u(x,0)=f(x) && (x\in\mathbb{R}) \\
	& u(x,t) \to 0 && (|x|\to\infty)
\end{empheq}
$u$は$x$について2回微分可能，$u,u',u''\in L^1(\mathbb{R})$と仮定．フーリエ変換を行う
\begin{empheq}[left = {\empheqlbrace \,}, right = {}]{align*}
	&\pdv{\hat u}{t}(\xi,t)=-\xi^2 u(\xi,t) && (\xi\in\mathbb{R},t>0) \\
	&\hat u(\xi,0)=\hat f(\xi) && (\xi\in\mathbb{R})
\end{empheq}
上の初期値問題の解は
\[
...
\begin{empheq}[left = {\empheqlbrace \,}, right = {}]{align*}
	&\pdv[2]{u}{x}+\pdv[2]{u}{y}=0 && ((x,y)\in D) \\
	&u(x,0)=f(x) && (x\in\mathbb{R}) \\
	& u(x,y) \to 0 && (y\to\infty)
\end{empheq}
$x$に関してフーリエ変換を行うと
